...
class LoadGamesJob < ApplicationJob
  ...
  def perform(*args)
    ...
    JSON.parse(make_request.body).each do |game_data|
      ...
      game["away_points"]      = game_data["awayPoints"] unless game_data["awayPoints"].blank?
      game["away_line_scores"] = game_data["awayLineScores"] unless game_data["awayLineScores"].blank?
      game["home_id"]          = Team.find_by(cfbd_id: game_data["homeId"])&.id
      game["home_points"]      = game_data["homePoints"] unless game_data["homePoints"].blank?
      game["home_line_scores"] = game_data["homeLineScores"] unless game_data["homeLineScores"].blank?
